...
$attachment  = '';
$attachment2  = '';
$attachmentH = utf8_decode("Prénom\tNom\tTéléphone\tEmail\tRue\tCode Postal\tVille\tDomaine\tDate dexpédition\tOptions\t\n");
...
if ($result->num_rows > 0) {
 ...
 while($row = $result->fetch_assoc()) {
	...
	$nacim = "";
          $attachment  .= $row2["nombre"]."\t".$row2["apellido"]."\t".$row2["telefono"]."\t".$row2["email"]."\t".$row2["direccion"]."\t".$row2["codp"]."\t".$row2["ciudad"]."\t".$dominio."\t".$row2["fecha"]."\t".$row2["opciones"]."\t\n";
      } 
    }else{
      $attachment2  = '';
      $dominio = ""; 
	  ...
	  $nacim2 = "";
          ...
          $rNl = $conn->query($sqlNl);
          $attachment2  .= $row3["nombre"]."\t".$row3["apellido"]."\t".$row3["telefono"]."\t".$row3["email"]."\t".$row3["direccion"].$row3["codp"]."\t".$row3["ciudad"]."\t"."\t".$dominio."\t".$row3["fecha"]."\t".$row3["opciones"]."\t\n";
      }
      if($attachment2  !== ''){
